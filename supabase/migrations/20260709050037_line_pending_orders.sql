...
--
-- 兩件撈回時發現的事，讀這支之前先知道：
--   1. 下面原始註解自稱「20250046」，但 repo 的 20250046 是 store_pages。
--      那個編號是當時寫的人腦中的序號，與 repo 實際狀態對不上，保留原樣不修。
--   2. 這張表服務的「LINE 全對話成單」設計**在 2026-07-13 已被推翻**，
--      改成連結制（bot 回預填結帳連結，走商城既有結帳流程）。
--      表還在 remote 上，但可能已經沒有任何程式在寫它。要刪之前先確認。
